...
────────────────────────────────────────
xxxxxxxxxxxxxx||||||||•••••••••••••••••••••••••••••|||||||||xxxxxxxxxxxxxxxxxx
──────────────────────────────────────── {m}•{k}•{h}•{sir} Author : RIZ_ANTI_SC_PREM {x}{m}•{k}•{h}•{x}'ok"')
#--------------------[ BAGIAN-MASUK ]--------------#
def login():
	try:
...
def login_lagi334():
	try:
		os.system('clear')
		banner()
		cetak(nel('\tMOHON MASUKAN COKOLATOS SIA : [green]NGARTI SIA[blue]'))
...
def setting():
	...
	if pwplus in ['y','Y']:
		pwpluss.append('ya')
		cetak('MASUKIN PW NYA TOD MINIMAL 6 HURUP\nExample :[green] memekk,jandaaa,hencett[white] ')
...
def passwrd():
	...
	print(f'>> MAINKAN MODE ANTIMO SETIAP 500 ID\n')
	...
	if woi in ['y','Y']:
		...
		print(f'\t{x}>>{k} DADAH KONTOLLLLLL {x} << ')
		time.sleep(2)
		back()
#--------------------[ METODE-MOBILE ]-----------------#
def crack(idf,pwv):
	...
	ua = 'Mozilla/5.0 (Linux; Android 9; SM-J415GN) AppleWebKit/537.36 (KHTML, seperti Gecko) Chrome/103.0.0.0 Mobile Safari/537.36'
